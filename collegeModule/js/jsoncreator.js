...
defaultFieldTypeJSON.autoLookup={
  ...
  "props":{
    ...
    $scope.createform = function() {
        $scope.formName = "sample Json";
        $scope.dataName = "Json";
        var newFieldJSON2 = angular.extend({}, formField1);        
        newFieldJSON2.formName = "sample1 json";
        $scope.fieldsJSON2 = formField1;
        //console.log($scope.fieldsJSON2);
        console.log(JSON.stringify($scope.fieldsJSON2));
        
        $scope.section = '';
           
    };
    
    $scope.addctrl = function(type) {       
        var newFieldJSON = angular.merge({}, defaultFieldJSON, defaultFieldTypeJSON[type]);
        newFieldJSON.type = type;
        $scope.fieldsJSON2.sections[0].fields.push(newFieldJSON);
        console.log($scope.fieldsJSON2);
        
    };
    $scope.fieldsJSON3 = [];
    $scope.showAll3 = function() {
        var newSection = angular.extend({},sectionField10);
        idCount["sections"]++;
         newSection.id = secIdPrefix + idCount["sections"];
        $scope.fieldsJSON2.sections.push(newSection);
         console.log(JSON.stringify( $scope.fieldsJSON2));
          $scope.fieldsJSON3.visible = true;            
        }
});     